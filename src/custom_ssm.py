...
class DoubleMLSSM(LinearScoreMixin, DoubleML):
    ...
    def __init__(self,
        ...
        ml_g_is_classifier_d1 = self._check_learner(ml_g_d1, 'ml_g_d1', regressor=True, classifier=True)
        ml_g_is_classifier_d0 = self._check_learner(ml_g_d0, 'ml_g_d0', regressor=True, classifier=True)
        _ = self._check_learner(ml_pi, 'ml_pi', regressor=False, classifier=True)
        _ = self._check_learner(ml_m, 'ml_m', regressor=False, classifier=True)

        self._learner = {
                            'ml_g_d0': clone(ml_g_d0),
                            'ml_g_d1': clone(ml_g_d1),
                            'ml_pi': clone(ml_pi),
                            'ml_m': clone(ml_m),
                        }
        ...
        if ml_g_is_classifier_d0:
            if self._dml_data._check_binary_outcome():
                self._predict_method['ml_g_d0'] = 'predict_proba'
            else:
                raise ValueError(f'The ml_g_d0 learner {str(ml_g_d0)} was identified as classifier '
    ...
    def _nuisance_est(self, smpls, n_jobs_cv, external_predictions, return_models=False):
        ...
        if self._score == 'missing-at-random':
            ...
            # conditional outcome
            g_hat_d1 = _dml_cv_predict(self._learner['ml_g_d1'], x, y, smpls=smpls_d1_s1, n_jobs=n_jobs_cv,
                                       est_params=self._get_params('ml_g_d1'), method=self._predict_method['ml_g_d0'],
                                       return_models=return_models)
            g_hat_d1['targets'] = g_hat_d1['targets'].astype(float)
            _check_finite_predictions(g_hat_d1['preds'], self._learner['ml_g_d1'], 'ml_g_d1', smpls)

            g_hat_d0 = _dml_cv_predict(self._learner['ml_g_d0'], x, y, smpls=smpls_d0_s1, n_jobs=n_jobs_cv,
                                       est_params=self._get_params('ml_g_d0'), method=self._predict_method['ml_g_d0'],
                                       return_models=return_models)
            g_hat_d0['targets'] = g_hat_d0['targets'].astype(float)
            _check_finite_predictions(g_hat_d0['preds'], self._learner['ml_g_d0'], 'ml_g_d0', smpls)
    ...
    def _nuisance_tuning(self, smpls, param_grids, scoring_methods, n_folds_tune, n_jobs_cv,
        ...
        if scoring_methods is None:
            scoring_methods = {'ml_g_d0': 'neg_mean_squared_error',
                               'ml_g_d1': 'neg_mean_squared_error',
                               'ml_pi': 'neg_log_loss',
                               'ml_m': 'neg_log_loss'}
        ...
        if self._score == 'nonignorable':
            ...
            def get_inner_train_inds(train_inds, d, s, random_state=42):
                ...
                for train_index in train_inds:
                    ...
                    stratify_vec = s_fold.astype(str) + d_fold.astype(str)
            ...
            for inner0, inner1 in zip(inner_train0_inds, inner_train1_inds):
                ...
                # 3. Predict proba on inner1
                pi_hat = ml_pi_temp.predict_proba(x_d_z[inner1])[:, 1]
                pi_hat_list.append((inner1, pi_hat))  # (index, value) tuple 

                # 4. Save best params
                pi_tune_ress.append(pi_tune_res[0])
            ...
            g_d0_tune_res = _dml_tune(y, x_pi_d, inner_train1_d0_s1,
                                    self._learner['ml_g_d0'], param_grids['ml_g_d0'], scoring_methods['ml_g_d0'],
                                    n_folds_tune, n_jobs_cv, search_mode, n_iter_randomized_search)
            g_d1_tune_res = _dml_tune(y, x_pi_d, inner_train1_d1_s1,
